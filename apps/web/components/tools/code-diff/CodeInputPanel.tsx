...
import { FullscreenButton, FullscreenPanel } from "@/components/tools/shared/FullscreenPanel";
import { useFullscreenPanel } from "@/components/tools/shared/useFullscreenPanel";
...
export function CodeInputPanel({ title, value, onChange, monacoLanguage, canFormat, onError }: CodeInputPanelProps) {
	...
	return (
		<FullscreenPanel isFullscreen={isFullscreen}>
			<div
				className={cn("relative flex flex-col overflow-hidden bg-card", isFullscreen ? "h-full" : "rounded-xl border border-border shadow-sm")}
				onDragOver={event => {
					event.preventDefault();
			...
			>
				<div className="flex shrink-0 items-center justify-between gap-2 border-b border-border bg-secondary px-4 py-2.5">
					...
					<div className="flex items-center gap-2">
						...
						<FullscreenButton isFullscreen={isFullscreen} onToggle={toggle} label={title} />
					</div>
				</div>

				<div className="min-h-0 flex-1">
					<CodeEditor value={value} onChange={onChange} language={monacoLanguage} height={isFullscreen ? "100%" : "380px"} placeholder="Paste, upload, or drop code here…" />
				</div>
